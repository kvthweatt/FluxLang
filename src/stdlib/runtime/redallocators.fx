// redheap.fx - Flux Custom Heap Allocator
//
...
#ifndef FLUX_HEAP_ALLOCATOR
#def FLUX_HEAP_ALLOCATOR 1;
    ...
    namespace memory
    {
        namespace allocators
        {
            ...
            def table_grow() -> bool
            {
                ...
                table_raw_insert(g_table, g_table_cap,
            ...
            {
                if (g_table == (BlockEntry*)0)
                {
                    if (!table_init()) { return false; };
                };

                // Grow if load > 50%
                if (g_table_count * (size_t)2 >= g_table_cap)
                {
                    if (!table_grow()) { return false; };
                };
            ...
            {
                if (g_table == (BlockEntry*)0) { return (BlockEntry*)0; };
            ...
            {
                if (g_table == (BlockEntry*)0) { return; };
            ...
            def heap_new_slab(size_t min_capacity) -> Slab*
            {
                ...
                if (min_capacity + slab_header_size() > sz)
                {
                    sz = min_capacity + slab_header_size();
                };

                u64* raw = heap_os_alloc(sz);
                if (raw == (u64)0) { return (Slab*)0; };
                ...
                if (next > g_slab_size_cap)
                {
                    next = g_slab_size_cap;
                };
            ...
            def bump_alloc(size_t bytes) -> u64
            {
                ...
                if (slab == (Slab*)0) { return (u64)0; };
                if (slab.frontier + bytes > slab.capacity) { return (u64)0; };
            ...
            def fmalloc(size_t size) -> u64
            {
                ...
                if (cls == (size_t)9)
                {
                    ...
                    if (raw == (u64)0) { return (u64)0; };

                    if (!table_insert(raw, size, (u64)1, raw))
                    {
                ...
                if (ptr == (u64)0)
                {
                    if (heap_new_slab(block) == (Slab*)0) { return (u64)0; };
                    ptr = bump_alloc(block);
                    if (ptr == (u64)0) { return (u64)0; };
                };

                if (!table_insert(ptr, cls, (u64)0, (u64)0))
                {
                    return (u64)0;
                };
            ...
            {
                if (ptr == (u64)0) { return fmalloc(new_size); };
                if (new_size == (size_t)0) { ffree(ptr); return (u64)0; };

                BlockEntry* entry = table_find(ptr);
                if (entry == (BlockEntry*)0) { return (u64)0; };
...
using standard::memory::allocators;
