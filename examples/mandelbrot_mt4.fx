...
// WorkSlice carries everything a thread needs for one frame's row band
struct WorkSlice
{
    int    row_start,
           row_end,
...
// Per-thread semaphores: main posts to wake a worker, worker posts done when finished
Semaphore[64] g_work_sem;
Semaphore[64] g_done_sem;

// Atomic shutdown flag: workers exit their loop when this is 1
i32 g_shutdown = 0;

// ============================================================================
//  Worker thread - persistent, loops until shutdown
// ============================================================================
...
def worker(void* arg) -> void*
{
    ...
    while (load32(@g_shutdown) == 0)
    {
        // Block until main signals a frame of work
        semaphore_wait(@g_work_sem[tid]);

        // Re-check shutdown after waking - main posts once more during teardown
        if (load32(@g_shutdown) != 0)
        {
            semaphore_post(@g_done_sem[tid]);
            return (void*)0;
        };

        WorkSlice* sl = @g_slices[tid];
...
def main() -> int
{
    ...
    // ── Initialise pool semaphores and spawn persistent workers ──
    int t;
    t = 0;
    while (t < num_threads)
    {
        semaphore_init(@g_work_sem[t], 0);
        semaphore_init(@g_done_sem[t], 0);
        t++;
    };
    ...
    while (t < num_threads)
    {
        thread_create(@worker, (void*)(i64)t, @threads[t]);
        t++;
    };
    ...
    while (win.process_messages())
    {
        ...
        // ── Fill slices and wake all workers ─────────────
    ...
    // ── Shutdown: set flag then wake all workers one last time ───
    store32(@g_shutdown, 1);
    t = 0;
    while (t < num_threads)
    {
        semaphore_post(@g_work_sem[t]);
    ...
    // Wait for all workers to confirm exit, then join and destroy
    t = 0;
    while (t < num_threads)
    {
        semaphore_wait(@g_done_sem[t]);
        thread_join(@threads[t]);
        semaphore_destroy(@g_work_sem[t]);
        semaphore_destroy(@g_done_sem[t]);
        t++;
    };
