...
// ============================================================================
// Double-double arithmetic using C-style shift/mask packing.
// A dd (double-double) is two floats packed into a u64: hi in bits 63:32, lo in bits 31:0.
// hi holds the main value, lo holds the error term.
// Together they give ~14 decimal digits of precision.
// ============================================================================

// Reinterpret a float's bits as a u32 (like *(uint32_t*)&f in C)
...
// Reinterpret a u32's bits as a float (like *(float*)&u in C)
def bits_to_float(u32 bits) -> float
{
    float* p = (float*)@bits;
    return *p;
};

// Pack two floats into a u64: hi occupies bits 63:32, lo occupies bits 31:0
def dd_pack(float hi, float lo) -> u64
{
    u64 hi_bits, lo_bits;
    hi_bits = (u64)float_to_bits(hi) `& 0xFFFFFFFF;
    lo_bits = (u64)float_to_bits(lo) `& 0xFFFFFFFF;
    return (hi_bits << 32) | lo_bits;
};

// Unpack hi part (bits 63:32)
...
// Unpack lo part (bits 31:0)
def dd_lo(u64 dd) -> float
{
    u32 bits;
    bits = (u32)(dd `& 0xFFFFFFFF);
    return bits_to_float(bits);
};
