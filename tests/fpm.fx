...
#def INTERNET_FLAG_IGNORE_CERT_CN_INVALID       0x00001000;
#def INTERNET_FLAG_IGNORE_CERT_DATE_INVALID     0x00002000;
#def INTERNET_QUERY_CONTENT_LENGTH              5;
#def HTTP_QUERY_STATUS_CODE                     19;
#def HTTP_QUERY_FLAG_NUMBER                     0x20000000;
#def INTERNET_OPTION_SECURITY_FLAGS             31;
#def SECURITY_FLAG_IGNORE_UNKNOWN_CA            0x00000100;
#def SECURITY_FLAG_IGNORE_CERT_CN_INVALID       0x00001000;
#def SECURITY_FLAG_IGNORE_CERT_DATE_INVALID     0x00002000;
...
extern
{
    def !!
        ...
        InternetSetOptionA(void*, u32, void*, u32) -> bool,
...
def http_get(byte* host, byte* path, byte** out_buf) -> int
{
    ...
    u32   bytes_read, total, cap, sec_flags, query_index;
    bool  ok;
    byte* buf;
    byte* newbuf;
    ...
    println("  [dbg] InternetOpenA OK");

    println(f"  [dbg] InternetConnectA to {host}:443...");
    ...
    println("  [dbg] InternetConnectA OK");

    println(f"  [dbg] HttpOpenRequestA {path}...");
    ...
    println("  [dbg] HttpOpenRequestA OK");

    // Relax certificate validation so self-signed/unknown CA certs don't crash
    sec_flags = SECURITY_FLAG_IGNORE_UNKNOWN_CA |
                SECURITY_FLAG_IGNORE_CERT_CN_INVALID |
                SECURITY_FLAG_IGNORE_CERT_DATE_INVALID;
    InternetSetOptionA(hrequest, INTERNET_OPTION_SECURITY_FLAGS,
                       @sec_flags, sizeof(u32) / sizeof(byte));
    println("  [dbg] InternetSetOptionA OK");

    println("  [dbg] HttpSendRequestA...");
    ...
    println("  [dbg] HttpSendRequestA OK");

    // Check HTTP status code
    status      = 0;
    status_size = 4;
    query_index = 0;
    println("  [dbg] HttpQueryInfoA...");
    HttpQueryInfoA(hrequest,
                   HTTP_QUERY_STATUS_CODE | HTTP_QUERY_FLAG_NUMBER,
                   @status, @status_size, @query_index);
    println(f"  [dbg] HTTP status = {status}");
    if (status != 0 & status != 200)
    {
        println(f"  ERROR: HTTP status {status}");
        InternetCloseHandle(hrequest);
        InternetCloseHandle(hconnect);
        InternetCloseHandle(hsession);
        return -1;
    ...
    cap   = FPM_HTTP_BUF;
    println(f"  [dbg] fmalloc({cap})...");
    buf   = (byte*)fmalloc((u64)cap);
    println(f"  [dbg] buf = {(u64)buf}");
    if ((u64)buf == 0)
    {
        println("  ERROR: fmalloc failed");
        InternetCloseHandle(hrequest);
        InternetCloseHandle(hconnect);
        InternetCloseHandle(hsession);
        return -1;
    ...
    bytes_read = 0;
    println("  [dbg] InternetReadFile loop...");
    ...
    println(f"  [dbg] read done, total = {total}");
    buf[total] = (byte)0;
...
def parse_json_file(byte* path, Arena* arena) -> JSONNode*
{
    ...
    root = (JSONNode*)alloc(arena, sizeof(JSONNode) / sizeof(byte));
...
def fetch_json_url(byte* host, byte* url_path, Arena* arena) -> JSONNode*
{
    ...
    if (n < 0) { println("> Err: n={n}"); return (JSONNode*)0; };
...
def main(int argc, byte** argv) -> int
{
    ...
    elif (strcmp(cmd, "update\0") == 0)
    {
        ...
        {
            println(f"> Err: remote_stdlib_root: {remote_stdlib_root}");
        };
