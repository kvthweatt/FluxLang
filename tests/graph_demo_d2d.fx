#import "standard.fx", "direct2d.fx", "d2d_graphing.fx";
...
def fill_circle(float* xs, float* ys, int count, float phase) -> void
{
    ...
    while (i < count)
    {
        t   = (float)i / (float)(count - 1) * 2.0 * PIF;
        xs[i]     = cos(t + phase);
        ys[i]     = sin(t + phase);
        i = i + 1;
    };
...
def fill_square(float* xs, float* ys, int count, float phase) -> void
{
    ...
    float s0 = sin(0.0 - phase), y_start;
...
def main() -> int
{
    D2DWindow win("direct2d.fx Demo\0", WIN_W, WIN_H, CW_USEDEFAULT, CW_USEDEFAULT);
    SetForegroundWindow(win.handle);

    D2DCanvas c(win.handle, (u32)WIN_W, (u32)WIN_H);

    // Allocate float arrays on the heap for each panel
    float* xs1 = (float*)fmalloc((u64)SAMPLE_COUNT * 4),
           ys1 = (float*)fmalloc((u64)SAMPLE_COUNT * 4),
           xs2 = (float*)fmalloc((u64)SAMPLE_COUNT * 4),
           ys2 = (float*)fmalloc((u64)SAMPLE_COUNT * 4);

    const int BAR_COUNT = 12;
    float* xs3 = (float*)fmalloc((u64)BAR_COUNT * 4),
           ys3 = (float*)fmalloc((u64)BAR_COUNT * 4);
    fill_bars(xs3, ys3, BAR_COUNT);

    const int SQ_COUNT = 10;
    float* xs4 = (float*)fmalloc((u64)SQ_COUNT * 4),
           ys4 = (float*)fmalloc((u64)SQ_COUNT * 4);
    ...
    DWORD col_sine      = RGB( 80, 200, 255),   // cyan
          col_area      = RGB( 40, 100, 128),   // dark cyan fill
          col_scatter   = RGB(255, 160,  40),   // orange
          col_bars      = RGB(100, 220, 130),   // green
          col_step      = RGB(220,  80, 160);   // pink
    ...
    while (win.process_messages())
    {
        ...
        // End Direct2D frame (presents to screen)
        c.end();
