...
def string_create(int initial_capacity) -> String
{
    ...
    s.length = 0;
    s.data = (byte*)malloc((u64)initial_capacity);
    return s;
};
...
def string_append(String* s, byte* cstr) -> void
{
    ...
    if (new_len >= s->capacity)
    {
        ...
        byte* new_data = (byte*)malloc((u64)new_cap);
        memcpy(new_data, s->data, (u64)s->length);
        free(s->data);
        s->data = new_data;
        s->capacity = new_cap;
    };
    
    memcpy(s->data + s->length, cstr, (u64)len);
    s->length = new_len;
    s->data[s->length] = 0;
...
def string_starts_with(String* s, byte* prefix) -> bool
{
    ...
    return strncmp(s->data, prefix, (u64)prefix_len) == 0;
};

def string_copy(String* s) -> String
{
    return string_from_cstr(s->data);
};
...
def string_array_create(int initial_capacity) -> StringArray
{
    ...
    arr.length = 0;
    arr.data = (String*)malloc((u64)(initial_capacity * sizeof(String)));
    return arr;
};
...
def string_array_push(StringArray* arr, String s) -> void
{
    if (arr->length >= arr->capacity)
    {
        ...
        String* new_data = (String*)malloc((u64)(new_cap * sizeof(String)));
        memcpy(new_data, arr->data, (u64)(arr->length * sizeof(String)));
        free(arr->data);
        arr->data = new_data;
...
def macro_table_create(int initial_capacity) -> MacroTable
{
    ...
    table.length = 0;
    table.data = (Macro*)malloc((u64)(initial_capacity * sizeof(Macro)));
    return table;
};
...
def macro_table_destroy(MacroTable* table) -> void
{
    ...
    {
        string_destroy(@(table->data[i].name));
        string_destroy(@(table->data[i].value));
        i++;
    };
...
def macro_table_set(MacroTable* table, byte* name, byte* value) -> void
{
    ...
    while (i < table->length)
    {
        if (string_equals(@(table->data[i].name), name))
        {
            // Update existing macro
            string_destroy(@(table->data[i].value));
            table->data[i].value = string_from_cstr(value);
            return void;
        };
        i++;
    };
    ...
    if (table->length >= table->capacity)
    {
        ...
        Macro* new_data = (Macro*)malloc((u64)(new_cap * sizeof(Macro)));
        memcpy(new_data, table->data, (u64)(table->length * sizeof(Macro)));
        free(table->data);
        table->data = new_data;
...
def macro_table_get(MacroTable* table, byte* name) -> byte*
{
    ...
    while (i < table->length)
    {
        if (string_equals(@(table->data[i].name), name))
        {
            return table->data[i].value.data;
        };
        i++;
    };
    return 0;
...
def file_set_create(int initial_capacity) -> FileSet
{
    ...
    set.count = 0;
    set.paths = (String*)malloc((u64)(initial_capacity * sizeof(String)));
    return set;
};
...
def file_set_add(FileSet* set, byte* path) -> void
{
    ...
    if (set->count >= set->capacity)
    {
        ...
        String* new_data = (String*)malloc((u64)(new_cap * sizeof(String)));
        memcpy(new_data, set->paths, (u64)(set->count * sizeof(String)));
        free(set->paths);
        set->paths = new_data;
...
def process_conditional_block(Preprocessor* pp, StringArray* lines, int start_idx, byte* macro_name, bool is_ifndef) -> int
{
    ...
    while (i < lines->length)
    {
        ...
        byte* stripped = trim_whitespace(line.data);
...
def process_line(Preprocessor* pp, StringArray* lines, int line_idx) -> int
{
    String line = lines->data[line_idx];
    byte* stripped = trim_whitespace(line.data);
    ...
    if (string_starts_with(@line, "#import\0"))
    {
        ...
        while (line.data[i] != 0)
        {
            if (line.data[i] == 34)  // Quote
            {
                i++;
                byte[256] import_file;
                int file_idx = 0;
                
                while (line.data[i] != 0 & line.data[i] != 34)
                {
                    import_file[file_idx] = line.data[i];
                    file_idx++;
                    i++;
                };
    ...
    String processed = substitute_macros(line.data, @(pp->macros));
...
def process_file(Preprocessor* pp, byte* filepath) -> void
{
    ...
    {
        byte c = stripped_content.data[i];
...
def preprocessor_process(Preprocessor* pp) -> void
{
    ...
    process_file(pp, pp->source_file.data);
    ...
    {
        String line = pp->output_lines.data[i];
        byte* trimmed = trim_whitespace(line.data);
    ...
    while (replaced)
    {
        ...
        while (j <= combined.length)
        {
            ...
            if (c == 10 | c == 0)
            {
                ...
                if (!string_equals(@substituted_line, original_line.data))
                {
                    replaced = 1;
                };
        ...
        {
            string_append(@combined, new_lines.data[j].data);
            string_append(@combined, "\\n\0");
            j++;
        };
    ...
    // Write output file
    int bytes_written = write_file("build\\tmp.fx\\0", combined.data, combined.length);
